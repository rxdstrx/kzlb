...
async function init() {
  try {
    ...
    // Supabase is AUTHORITATIVE for country membership — it reflects flag changes / moves
    // instantly (the GitHub cache files can lag, so they must never decide membership).
    try {
    ...
    // Cache only supplies maps_list (Supabase doesn't store it) — enriches existing rows,
    // never adds members. Falls back to the cache list only if Supabase failed.
    try {
...
async function renderByMap(mapName) {
  ...
  // Apply role filter
  if (ptRoleFilter !== 'all') {
    sorted = ptRoleSteamids.size > 0 ? sorted.filter(p => ptRoleSteamids.has(p.steamid)) : [];
  }
