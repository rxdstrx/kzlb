...
allPlayers.forEach(p => {
  if (fs.existsSync(path.join(cacheDir, `${p.steamid64}.json`))) alreadyScraped.add(p.steamid64);
});
...
// Direct HTTP fetch using cookies (no browser needed per request)
function fetchStats(steamid, cookieHeader) {
...
async function processBatch(batch, cookieHeader, offset) {
  return Promise.all(batch.map(async (player, idx) => {
    const { steamid64, nickname, faceit_elo, skill_level } = player;
    try {
      const maps = await fetchStats(steamid64, cookieHeader);
      ...
      if (mapList.length > 0) {
        ...
        console.log(`[${offset + idx + 1}/${players.length}] ✓ ${nickname} — ${mapList.length} maps, ${result.kz_points} pts`);
        return result;
      } else {
        console.log(`[${offset + idx + 1}/${players.length}] ✗ ${nickname}`);
        return null;
      }
    } catch (e) {
      console.log(`[${offset + idx + 1}/${players.length}] ERROR ${nickname}: ${e.message}`);
      return null;
    }
  }));
}
...
(async () => {
  // Step 1: use browser ONCE to get Cloudflare session cookies
  console.log('Launching browser to get Cloudflare session...');
  ...
  console.log(`Got ${allCookies.length} cookies. Starting parallel scrape (${CONCURRENCY} concurrent)...\n`);

  // Step 2: scrape all players in parallel batches (no browser needed)
  const kzPlayers = [];
  let withKZ = 0, withoutKZ = 0;

  for (let i = 0; i < players.length; i += CONCURRENCY) {
    const batch = players.slice(i, i + CONCURRENCY);
    const results = await processBatch(batch, cookieHeader, i);
    for (const r of results) {
      if (r) { kzPlayers.push(r); withKZ++; } else { withoutKZ++; }
    }
    // Small delay between batches to avoid rate limiting
    if (i + CONCURRENCY < players.length) await new Promise(r => setTimeout(r, 500));
  }
